...
echo ============Start Linking============
link "%objpath%\hv_stack\main.obj" "%objpath%\hv_stack\pvcvm.obj" "%objpath%\hv_stack\pvmisc.obj" "%objpath%\hv_stack\debug_agent.obj" "%objpath%\hv_stack\version.res" /LIBPATH:"%libpath%\um\x64" /LIBPATH:"%libpath%\ucrt\x64" /LIBPATH:"%ddkpath%\lib\x64" /LIBPATH:"%dbgpath%\lib\x64" "ntdll.lib" "dbghelp.lib" "..\header\lib\x64\NoirCvmApi.lib" /NOLOGO /DEBUG /INCREMENTAL:NO /PDB:"%binpath%\NoirPvCuda.pdb" /OUT:"%binpath%\NoirPvCuda.exe" /SUBSYSTEM:CONSOLE /Machine:X64 /ERRORREPORT:QUEUE
